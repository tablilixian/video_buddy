...
import {
  ...
  MAX_TARGET_DURATION,
  suggestShotCount,
  type StudioPlanAspectRatio,
  type StudioProjectPlan,
  type StudioWorkflowMode,
} from './contracts/project.js'
import { DEFAULT_RESOLUTION, isVideoResolution } from './config.js'
import type { VideoResolution } from './providers/types.js'

/** 兜底画幅（项目未预置、设置页也没读到时的最后一层）。 */
export const FALLBACK_ASPECT_RATIO: StudioPlanAspectRatio = '16:9'
 ...
 */
export const FALLBACK_TARGET_DURATION = 30

/** 兜底分辨率档位（与设置项 schema 的 default 同源，不另写字面量）。 */
export const FALLBACK_RESOLUTION: VideoResolution = DEFAULT_RESOLUTION
...
function isPlanAspectRatio(value: string | undefined): value is StudioPlanAspectRatio {
  return value !== undefined && (PLAN_ASPECT_RATIOS as readonly string[]).includes(value)
}

/**
 * 解析放手跑要用的规格。三层优先级逐项独立生效（画幅走预置不代表时长也走预置）。
 */
export function resolveStudioDefaults(input: StudioDefaultsInput = {}): StudioDefaults {
  const plan = input.plan
  const settings = input.settings ?? {}

  const plannedAspect = plan?.aspectRatio
  const settingsAspect = settings.defaultAspectRatio
  const aspectRatio: StudioPlanAspectRatio = isPlanAspectRatio(plannedAspect)
    ? plannedAspect
    : isPlanAspectRatio(settingsAspect) ? settingsAspect : FALLBACK_ASPECT_RATIO
  const aspectRatioSource: StudioDefaultSource = isPlanAspectRatio(plannedAspect)
    ? 'plan'
    : isPlanAspectRatio(settingsAspect) ? 'settings' : 'fallback'

  // 预置时长可能来自直连调用（脏值），此处再收口一次：非法即视为未预置。
  const plannedDuration = plan?.targetDuration
  const hasPlannedDuration = typeof plannedDuration === 'number'
    && Number.isFinite(plannedDuration) && plannedDuration > 0
  const targetDuration = hasPlannedDuration
...
/** 层名（进文案；与 `StudioDefaultSource` 一一对应，不另开第二套映射）。 */
const SOURCE_LABELS: Readonly<Record<StudioDefaultSource, string>> = {
  plan: '项目预置',
  settings: '设置页',
  fallback: '兜底',
}
 ...
 */
export function describeStudioDefaults(defaults: StudioDefaults): string {
 ...
 * 「超时」与「放手跑」两条路上给出不同答案。
 */
export function recommendedOptionOf(options: readonly string[]): string | undefined {
