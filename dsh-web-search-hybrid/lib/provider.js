...
const FREE_SEARCH_ENGINES = [
    {
        name: 'bing',
        run: (request, options, signal) => searchBing(request.query, request.maxResults ?? 8, { bingMarket: options.bingMarket }, signal),
    },
    {
        name: 'ddg-lite',
        run: (request, _options, signal) => searchDdgLite(request.query, request.maxResults ?? 8, {}, signal),
    },
    {
        name: 'ddg-html',
        run: (request, _options, signal) => searchDdgHtml(request.query, request.maxResults ?? 8, {}, signal),
    },
];
/** Run the keyless free-engine chain; throws `WEB_PROVIDER_ERROR` when every engine fails. */
async function freeSearch(request, options, signal) {
    const failures = [];
    for (const engine of FREE_SEARCH_ENGINES) {
        if (signal?.aborted)
            throw new WebError('hybrid search aborted', 'WEB_ABORTED');
        try {
            ...
            if (signal?.aborted)
                throw new WebError('hybrid search aborted', 'WEB_ABORTED', { cause: error });
...
export class HybridSearchProvider {
    ...
    async search(request, signal) {
        ...
        if (apiKey === undefined || apiKey.length === 0)
            return freeSearch(request, this.options, signal);
        try {
...
export class HybridFetchProvider {
    options;
    id = HYBRID_PROVIDER_ID;
    tinyfish;
    constructor(options, tinyfishOptions) {
    ...
    async fetch(request, signal) {
        const apiKey = await this.options.resolveApiKey();
        if (apiKey === undefined || apiKey.length === 0)
            return httpFetch(request, signal);
        try {
 ...
 */
async function httpFetch(request, signal) {
    if (signal?.aborted)
...
async function followAndRead(initialUrl, signal) {
...
async function readBody(response, finalUrl) {
