...
| 3D 动画短片 | 完整风格化 3D 动画 | 走标准动画流程：简报→大纲→角色/环境设定卡→镜头规划→故事板→逐镜→合成配乐；强调角色一致、场景连续、节奏精准。定妆锚点用 3D 角色设定图。**完整流程用 skill 工具加载 \`3d-animation-short-generator\`（原版 STEP 0–9：简报/大纲/角色卡/场景卡/六列镜头表/自检门/分镜/模型选择/逐镜/拼接+BGM），按其中选项卡门逐项确认；BGM/配音/字幕步骤用 music_generation / tts_voiceover / subtitle_burn 占位工具获取降级指引。** |
