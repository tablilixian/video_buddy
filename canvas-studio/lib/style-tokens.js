...
export const LOOK_TOKEN_KEYS = ['色彩', '光线', '材质', '镜头语汇', '节奏'];
...
function isLookTokenKey(value) {
    return LOOK_TOKEN_KEYS.includes(value);
}
/**
 ...
 * 而不是把生成流程打断。
 */
export function parseLookTokens(text) {
    const out = {};
    for (const raw of text.split(/\r?\n/u)) {
        ...
        if (value.length === 0 || !isLookTokenKey(key))
            continue;
 ...
 */
export function mergeLookTokens(texts) {
