...
7. **Video model strategy**
   - This project has no selectable video model: always use MiniMax-H3, because it often produces cleaner Apple-like product close-ups, negative space, and premium product-film camera language.
   ...
   - Never offer a model switch at any stage; if H3 fails, retry or loosen the request instead of changing models.
...
- Video generation defaults to MiniMax-H3 native audio unless the user explicitly asks for silence. Use the later Apple-style tech BGM direction as the audio prompt so picture rhythm and sound are generated together.
...
- MiniMax-H3 native audio setting and music prompt; native audio is on by default unless the user explicitly asks for silence.
