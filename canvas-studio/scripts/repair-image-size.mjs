/**
 * CV-229 存量画布修复：把「从没量过真实分辨率」的图片节点补上尺寸与分辨率。
 *
 * ⚠️ **备用工具，不是流程环节 —— 默认不要跑。**
 * 项目处于开发阶段，约定是**只保证新产出的数据正确、不回填历史数据**
 * （2026-09-22 用户明确）。修产生路径的那一半已经在 Host 落盘侧做完
 * （`generate.ts` 的 character_sheet 与 `video-frames.ts` 都改为落盘即写实测尺寸），
 * 本脚本只在用户**点名**要求修某批存量画布时才用。
 * 跑之前请先退出桌面应用：应用内存里的旧画布保存一次就会覆盖本脚本的写盘。
 *
