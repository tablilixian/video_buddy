...
> 纯函数与单测保留）。取证中还**定位并修掉了第二个缺陷** —— `isCanvasNode` 白名单漏 `'group'`
> 导致组节点被读取层吞掉、成员 `parentId` 悬空、图层面板丢 7 行（**CV-172**，含回归用例与
> 真实数据端到端验证）；另**排除了一条假线索**（Chromium 磁盘缓存）。详见 **§9**。
>
...
- ~~7 个提交是否 push~~ → 已 push。
- **已结案**：§9.2 的根因（`isCanvasNode` 白名单漏 `'group'`）已按方案 **A 修掉（CV-172）**，
  组节点可正常落盘/读回。**待真机复验**：新生成一张分镜关键帧，确认画布出现「分镜 N · 素材」组框、
  图层面板不再少行。
...
### 9.2 ✅ 已结案并已修（CV-172，09-13 22:45）：组节点被读取层吞掉 → 幽灵 parentId → 面板丢行
...
**修复选项（用户 09-13 22:45 拍板选 A）**：

| 方案 | 状态 |
|---|---|
| **A. 修白名单**（恢复自动编组） | ✅ **已实施 —— CV-172** |
| B. 明确不要自动编组 | 未采用 |

**CV-172 落地内容**：

- `src/projects.ts` 的 `isCanvasNode` 白名单补上 `|| node.kind === 'group'`（含事故链注释）。
- 新增回归用例 `tests/projects-dir.test.mjs`：「CV-172：组节点能落盘并读回」。
- **断言有效性已实证**：把编译产物里的那条分支删掉 → `pass 0 / fail 1`；装回 → `pass 17 / fail 0`。
  （「通过」不等于「拦得住」，这一步不能省。）
- **真实数据端到端**：用户的 18 节点 + 注入 1 个组 → 写 19 读回 **19**（组存活）；
  再模拟一次客户端保存（只带 18 个节点）→ 读回 **19**（`writeCanvas` 的合并保护把组留住）。
- 验证链：`build` OK、**536/536** 测试、双端 `typecheck` exit 0。
- ⚠️ **预期内的副作用**：组节点从此能存活 ⇒ 新生成的分镜产物会自动归入「分镜 N · 素材」组，
  画布上会**新增半透明组框**（`zIndex:-1`）。历史项目里的组**已被抹除且不可恢复**，
  所以旧项目外观不变 —— 只有新生成的产物才会带上组框。
- 随之激活的相关路径（此前是死代码）：`CanvasNode.tsx:532` 组框渲染、
  `CanvasContextMenu.tsx:85`「解组」、`StudioFrame.tsx:982` 组操作。
