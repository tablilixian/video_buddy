...
const dispatchers = new Map<number, unknown>()
let warned = false
...
export function longRequestDispatcher(timeoutMs: number = LONG_REQUEST_TIMEOUT_MS): unknown | undefined {
  ...
  try {
    const current = (globalThis as unknown as Record<symbol, unknown>)[GLOBAL_DISPATCHER_SYMBOL]
    const ctor = (current as { constructor?: unknown } | null | undefined)?.constructor
    if (typeof ctor !== 'function') {
      warnOnce()
      return undefined
    }
    const dispatcher = new (ctor as new (options: Record<string, unknown>) => unknown)({
    ...
    if (typeof (dispatcher as { dispatch?: unknown } | null | undefined)?.dispatch !== 'function') {
      warnOnce()
      return undefined
    }
