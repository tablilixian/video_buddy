 ...
 */
export const QC_AUTO_MODE_NOTICE =
  '放手跑模式下逐镜质检已关闭（一致性由锁定提示词逐字节注入与参考图锚点保障，本调用未执行、不消耗重跑预算）。'
 ...
 */
export function hasLookTokenBaseline(expect: string): boolean {
 ...
 */
export function buildQcPrompt(expect: string): string {
  const lines = [
    '对照【期望描述】逐项核对画面中的固定要素：人物外貌（脸型/发型/发色）、服装（款式/颜色）、核心道具、整体配色与光感。',
  ]
  if (hasLookTokenBaseline(expect)) {
    lines.push(
      '期望描述里含【风格基准（Look）】的 5 项 tokens：除角色要素外，再逐项核对 4 个单帧可判的风格维度 —— 色彩与调色是否同调、光线方向与软硬是否一致、材质质感是否同类、镜头语汇（景深/机位感/框景）是否同法。风格漂移项请点名对应的 token 行（如「光线」）。',
      '注意：「节奏」是跨镜的时间维度，单帧无法判定，不要因它给 FAIL。',
    )
  }
  lines.push(
    '只回答如下 JSON：',
 ...
 * expect，避免无基准瞎判）。
 */
export function defaultQcExpect(assets: readonly StudioAsset[] | undefined): string {
  if (assets === undefined || assets.length === 0) return ''
  const fmt = (asset: StudioAsset): string =>
    `[${asset.name}] ${asset.lockedPrompt}${asset.negativePrompt !== undefined && asset.negativePrompt.length > 0 ? `（禁止：${asset.negativePrompt}）` : ''}`
 ...
 */
export function parseQcVerdict(raw: string): QcVerdict {
  ...
  const drifts = Array.isArray(json.drifts)
    ? json.drifts.filter((d): d is string => typeof d === 'string' && d.trim().length > 0).map((d) => d.trim()).slice(0, MAX_DRIFTS)
    : []
...
function normalizeVerdict(value: unknown): QcVerdictValue {
  const v = String(value ?? '').trim().toUpperCase()
  // 先判否定式：中文「不通过」「未通过」都含「通过」，顺序反了会被误判为 PASS。
  if (v.includes('FAIL') || v.includes('不通过') || v.includes('未通过') || v.includes('漂移')) return 'fail'
  if (v.includes('WARN') || v.includes('无法') || v.includes('模糊')) return 'warn'
  if (v.includes('PASS') || v.includes('通过') || v.includes('一致')) return 'pass'
  return 'warn'
}
 ...
 */
export function pickQcTargetNode(nodes: readonly StudioCanvasNode[], filename: string): StudioCanvasNode | null {
  const hits = nodes
 ...
 */
export function nextQcAttempt(
  nodes: readonly StudioCanvasNode[],
  target: StudioCanvasNode | null,
  shotCardIds: readonly string[] = [],
): number {
  const inLineage = (node: StudioCanvasNode): boolean =>
    shotCardIds.length === 0
 ...
 */
export async function runShotQc(
  nodes: readonly StudioCanvasNode[],
  filename: string,
  options: QcShotOptions,
): Promise<QcShotResult> {
