 ...
 */
export function fadeAnchorOf(bgmDuration, filmDuration) {
 ...
 */
export function bgmShortfallMessage(bgmDuration, filmDuration) {
    // 无法判定时不报错（调用方会降级并留 warning），避免把「探测失败」当成「BGM 太短」。
    if (!(filmDuration > 0) || !(bgmDuration > 0))
        return null;
    const shortfall = filmDuration - bgmDuration;
    // 浮点容差：两个时长都来自 ffprobe 的小数解析（如 15.5 − 15.45 在二进制下差 7e-16），
    // 恰好在阈值上时不该因为浮点误差报错。
    if (shortfall <= BGM_SHORTFALL_TOLERANCE_SEC + 1e-6)
        return null;
 ...
 */
export function buildAmixArgs(concatOutput, bgmInput, output, hasConcatAudio, bgmDuration = 0, filmDuration = 0) {
 ...
 */
export function auditTimeline(input) {
    ...
    if (clipDeclared > 0 && filmDuration > 0 && Math.abs(filmDuration - clipDeclared) > tolerance) {
        notices.push(`逐镜请求时长合计 ${clipDeclared}s，成片真值 ${filmDuration.toFixed(3)}s（相差 ${(filmDuration - clipDeclared).toFixed(3)}s）——`
    ...
    if (storyboardDeclared > 0 && clipDeclared > 0 && Math.abs(clipDeclared - storyboardDeclared) > tolerance) {
        notices.push(`分镜表声明时长合计 ${storyboardDeclared}s，但实际生成用了 ${clipDeclared}s（相差 ${(clipDeclared - storyboardDeclared).toFixed(3)}s）——`
    ...
    if (targetDuration !== undefined && targetDuration > 0 && filmDuration > 0 && Math.abs(filmDuration - targetDuration) > tolerance) {
        notices.push(`成片真值 ${filmDuration.toFixed(3)}s 与项目目标总时长 ${targetDuration}s 相差 ${(filmDuration - targetDuration).toFixed(3)}s。`);
    }
    return notices;
}
...
export async function composeStudioVideo(registry, projectId, clipIds, bgmNodeId, options = {}, signal) {
    ...
    try {
        ...
        if (bgmNodeId !== undefined) {
            ...
            const amixResult = await runFfmpeg(ffmpegPath, buildAmixArgs(concatOutput, bgmInput, finalOutput, hasConcatAudio, bgmDuration, filmDuration), COMPOSE_TIMEOUT_MS, composed);
        ...
        const audioComposition = bgmNodeId !== undefined
