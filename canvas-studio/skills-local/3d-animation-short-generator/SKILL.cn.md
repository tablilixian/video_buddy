...
description: |
  将一句故事创意转化为完整的风格化3D动画短片制作流程，依次完成项目简报、故事大纲、角色卡、无人物场景卡、标准镜头表、文本分镜或可选铅笔分镜、单镜头生成（视频模型固定 MiniMax H3）、全片拼接、BGM匹配与最终复查。适用于需要角色一致、场景连续、节奏明确和音画配合的剧情动画短片、纪念短片、品牌故事与社交媒体内容。不适用于单张图片、简单修图、真人写实视频或单一独立镜头。
---
...
核心规则：**先故事、再资产；用户输入内容后立刻用选项卡确认画面尺寸/比例与总时长；产物按顺序上画布；所有需要用户确认的地方必须用选项卡；角色/场景卡后固定顺序为：六列标准镜头信息表（每秒指令+音频+空间锚点链+hook）→ 镜头表自检门 → 一份单一文本分镜文档（每镜一节，对齐半解说短剧分镜字段；用户主动开启可视化模式时再加铅笔图；用户标记需要重点迭代的镜时抽取成独立节点）→ 用 MiniMax H3（固定，无模型/分辨率选择）生成单镜头片段 → 全片拼接与 BGM 合成；最终视频必须清除所有分镜痕迹**。

## 全局视觉风格锁

除非用户明确指定其他视觉风格，所有角色卡、场景卡、镜头表格、文本分镜、可选铅笔分镜、单镜头视频片段、拼接正片和最终合成都必须使用以下视觉风格：
...
- 一句话创意或粗略故事
- 目标产物：只要蓝图、只要资产、六列标准镜头信息表、单文本分镜文档（默认）+ 重点迭代镜抽取的独立节点 + 多宫格铅笔分镜（开启可视化模式时）、单镜头视频片段（MiniMax H3 渲染）、拼接正片，还是 BGM 合成最终成片
- 用户已说明的预估时长
- 用户已说明的画面尺寸 / 比例
- 视觉基调：默认温暖风格化 3D 动画
- 台词需求：有对白、旁白，还是无台词
...
- **镜头描述**：景别、镜头运动、荷兰角设计、表演风格、音效、负向提示、**视频模型生成备注**（视频模型固定 MiniMax H3 — 强调包装关键词和文字/UI/动效清晰度），并强制包含 `Per-Second Directives` 子节。子节必须按秒拆解为 `0–1s`、`1–2s`、`2–3s` 等指令；亚秒关键节拍用 `2.0–2.5s` 风格标记。每条每秒指令必须覆盖全部 5 个必填要素：
...
- **单文本分镜文档（默认，推荐）** — 整个短片一份画布文本节点，所有分镜作为文档内章节。对齐半解说短剧分镜结构：每镜字段（标题/hook/场景/角色/空间锚点/连续性/表演备注）加 Pixar 的每面板四象限内容 + 可选 ASCII 布局。承载完整质控载荷，成本几乎为零。MiniMax H3 直接读取作为该镜渲染参考。
...
- 项目标题、分镜模式、自检状态（如 `shot-table self-check: passed at <时间戳>`）。
...
## STEP 7：单镜头视频片段（视频模型固定 MiniMax H3）

### 无视频模型 / 分辨率选择

本项目**没有可选择的视频模型**：所有片段统一用 **MiniMax H3** 渲染。不要展示视频模型选项卡，不要问用户「用 H3 还是其他模型」，也不要提供逐镜混合模式。

分辨率同样不可选：不要展示分辨率选项卡、不要向用户提问分辨率，按后端默认渲染，并保持项目简报中已批准的画面尺寸 / 比例。

### 单镜头片段渲染

每行批准后调用 MiniMax H3 生成对应独立视频片段。每条片段必须严格使用单文本分镜文档中匹配的章节（若该章节已抽取成独立节点则从独立节点取）、角色卡和场景卡。
...
视频模型固定为 MiniMax H3，任何回退路径都不得提供「切换模型」选项。
...
如果渲染片段与已批准的 `参考锚点` 漂移（门框落到错边、人物从错边离屏、光位翻转），用直接引用 `参考锚点` 块的强化提示重渲染；不要在拼接时静默混合修正版和未修正版。
...
- 单镜头视频片段渲染前：确认渲染清单（模型固定 MiniMax H3，无模型/分辨率选择）
- 单镜头视频片段后
- 全片拼接、BGM 匹配和最终合成后
...
- 单镜头视频片段重渲染后，拼接、BGM 匹配和最终合成必须用新片段。
