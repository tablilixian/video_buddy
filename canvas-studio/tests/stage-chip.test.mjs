/**
 * DD-09 / c 会话头「制作阶段」chip 的判定契约（stage-chip.ts）。
 *
 * 这条判定决定**会话头右侧那个胶囊出不出现、显示第几段、是不是在等你拍板** ——
 * 每次渲染都在跑，而且出错的方式很刺眼（lobby 首屏右侧凭空多一个「剧本 1/6」，
 * 或者在用户已经批准之后还挂着 gold 的「等你拍板」）。所以口径钉在纯函数上。
 *
 * 每个用例对应一条真实边界：
 *
 * 1. **未选项目 = 一个 DOM 都不出** —— 宿主那排 utilities 是 `:empty` 折叠的，
 *    但如果渲染出一个空壳，`:empty` 不成立、`margin-left:20px` 照算，会话头右侧
 *    会留一道空白。所以「不显示」必须是 `null`，不是「渲染个空 span」。
 * 2. **阶段序号不重新推导** —— 复用 `deriveWorkflowStage`。本模块只把它翻译成
 *    chip 的话，不许自己看 `workflow.state` 是不是某个 review 值（那是第二份判定）。
 * 3. **待批准 = 需要打断用户**，是 chip 变 gold 的唯一条件。
