...
function outcomeWith(
  base: { url: string; filename?: string },
...
export async function runVideo(
  ...
  if (handle.settled !== undefined) {
    ctx.onProgress?.(1)
    return outcomeWith(
      handle.settled.filename !== undefined
  ...
  for (;;) {
    ...
    if (polled.done) {
      ctx.onProgress?.(1)
      return outcomeWith(
        polled.filename !== undefined ? { url: polled.url, filename: polled.filename } : { url: polled.url },
        handle,
      )
    }
