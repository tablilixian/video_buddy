...
import { generateAsset, assetKeyFromUrl, promoteAssetFile, uploadImage, enhancePrompt, analyzeImage, isDramaProductName, generateCharacterSheet, generateMusic, setRuntimeConfig, deriveNodePlacement, clampDuration, type GenerateParams, type GenerateResult, type CharacterSheetResult, type MusicResult } from './generate.js'
  ...
  return [
    ...
    defineTool({
      ...
      parameters: {
        ...
        filename: { type: 'string' as const, description: '可选：首帧图的**句柄**（upload_image 返回，或 @ref[显示名]——Host 会把画布节点上的产物名自动换成句柄），用作视频首帧；不传则为纯文生视频。⚠️ 生成工具结果里的产物名（形如 img_01287_.png）不能直接传' },
    ...
    defineTool({
      ...
      parameters: {
        ...
        filenames: { type: 'array' as const, required: true, description: '参考图的**句柄**数组（upload_image 返回，或 @ref[显示名]——Host 会把画布节点上的产物名自动换成句柄）。⚠️ 生成工具结果里的产物名不能直接传。上限由供应商决定：Drama 6 张、fal 9 张，超出自动采样（保留首尾）' },
        aspectRatio: { type: 'string' as const, enum: ['16:9', '9:16'], description: '宽高比，默认 16:9。视频只有横屏 16:9 与竖屏 9:16 两档' },
