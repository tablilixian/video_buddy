  ...
  if (variant === 'choice') {
    return (
      <div
        ...
        role="group"
        {...(labelledBy === undefined ? { 'aria-label': ariaLabel ?? '执行模式' } : { 'aria-labelledby': labelledBy })}
      >
        {MODES.map(value => (
          <button
            key={value}
            type="button"
            className="csChoice"
            // 选中态走 aria-pressed（与同排的「画幅 / 目标时长」同一机制）——
            // 样式只读 DOM 属性，不会出现「类挂了但没同步属性」。
            aria-pressed={mode === value}
            disabled={disabled}
            title={mode === value ? MODE_COPY[value].current : MODE_COPY[value].switchTo}
            onClick={() => { if (value !== mode) onChange(value) }}
          >
            <span className="csChoiceMain">{MODE_COPY[value].main}</span>
            <span className="csChoiceSub">{MODE_COPY[value].sub}</span>
          </button>
        ))}
      </div>
    )
