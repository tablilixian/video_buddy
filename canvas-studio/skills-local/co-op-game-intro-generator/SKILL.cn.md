...
## STEP 5：等待用户确认
首图生成后，**立即调用 `ask_user_choice`** 求确认（问题如「首图是否确认？」，选项：`确认，生成视频（推荐）`、`修改首图`），不要用纯文本问句求确认。在用户通过选项卡确认首图或提出修改之前，不要生成视频。如果用户改了风格、名字、游戏名、角色形象或首图问题，就回到首图提示词阶段重写并再生成。

## STEP 6：回填视频 prompt，并用 Minimax H3 生成
首图确认后，再把确认结果回填进完整视频 prompt，包括风格、角色参考、玩家名字、游戏名字、UI 文案和负面约束，然后直接用 Minimax H3 生成最终视频（视频模型固定为 MiniMax H3，无模型选择环节）。
