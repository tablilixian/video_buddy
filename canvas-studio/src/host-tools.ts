...
const lookCardSchema = {
  ...
  additionalProperties: false,
  properties: {
    assetId: { type: 'string' as const, description: '建立的 Look 卡 id' },
    name: { type: 'string' as const, description: 'Look 卡显示名（已归一，含 `Look · ` 前缀）' },
    lockedPrompt: { type: 'string' as const, description: '冻结后的 5 项 tokens（逐镜逐字节复用；补齐字段时同名重调本工具覆盖）' },
    anchors: { type: 'array' as const, description: '锚点摘要（title / url / 后端产物名）；无锚点时为[]。要把它作参考图，请用 @ref[title]' },
    warnings: { type: 'array' as const, items: { type: 'string' as const }, description: '非致命提示（tokens 未解析到全部 5 项 / 锚点没对应上画布节点）' },
  },
}
...
function renderLookCardResult(_args: unknown, value: unknown): ContentBlock[] {
  const v = value as LookCardResult
  const lines = [
 ...
 */
export function parseShotDurationSeconds(text: string): number {
...
const SHOT_CARD_SIZE = { width: 360, height: 220 } as const
 ...
 */
export function shotCardNumberOf(title: string | undefined): number | undefined {
 ...
 */
function placeShotCard(
  occupied: readonly StudioCanvasNode[],
  ...
  for (let step = 0; step < PLACEMENT_SCAN; step += 1) {
    ...
    if (!occupied.some((node) => boxesOverlap({ ...position, ...SHOT_CARD_SIZE }, node))) return position
  }
 ...
 */
export function mergeShotCards(
  ...
  const byNumber = new Map<number, StudioCanvasNode>()
  for (const node of existing) {
    if (node.toolName !== STORYBOARD_NODE_TOOL) continue
    const no = shotCardNumberOf(node.title)
    if (no === undefined || byNumber.has(no)) continue
    byNumber.set(no, node)
  }

  const base = deriveNodePlacement(existing, sourceIds, SHOT_CARD_SIZE.width, SHOT_CARD_SIZE.height)
  const occupied: StudioCanvasNode[] = [...existing]
  const cards: StudioCanvasNode[] = []
  const created: StudioCanvasNode[] = []
  ...
  return [
    ...
    defineTool({
      ...
      parameters: {
        ...
        shotRefs: { type: 'array' as const, description: '可选：要关联的分镜卡（「分镜 N · 景别」标题、「分镜 N」镜号或节点 id）。修复逐镜关键帧上的文字时应传' },
      },
      output: { schema: resultSchema, render: renderResult },
      async execute(args, exec) {
      ...
      output: {
        schema: {
          ...
          properties: {
            ...
            filename: { type: 'string' as const, description: '四视图拼图的后端产物名（**不可**直接作生成工具的 filenames；要作参考请用 @ref[节点标题]）' },
          },
        },
    ...
    defineTool({
      ...
      parameters: {
        ...
        referenceFilename: { type: 'string' as const, description: '可选：样张 / 参考图，作本卡的视觉锚点。可传 `@ref[节点标题]`、画布节点 id，或 upload_image 得到的文件名（指向 ②-2 已落画布的那张样张最省事）' },
        negativePrompt: { type: 'string' as const, description: '可选：风格负面约束（如「不要高饱和」「不要现代玻璃幕墙」）' },
      },
      output: { schema: lookCardSchema, render: renderLookCardResult },
      async execute(args, exec) {
        const a = args as { name: string; lockedPrompt: string; referenceFilename?: string; negativePrompt?: string }
        const projectId = await resolveProjectId(registry, exec.agent?.session.header.cwd)
    ...
    defineTool({
      ...
      parameters: {
        ...
        filename: { type: 'string' as const, required: true, description: '被检镜头图的**可用句柄**（`upload_image` 的返回，或 `@ref[显示名]`——Host 会把画布节点上的产物名自动换成句柄）。⚠️ 不要把生成工具结果里的产物名（形如 img_01287_.png）直接传进来。' },
      ...
      async execute(args, exec) {
        ...
        const result = await runShotQc(doc.nodes, filename, {
          ...
          analyze: (name, prompt, systemPrompt, signal) =>
            analyzeImage(name, prompt, systemPrompt, signal, { registry, projectId }),
          expect,
          shotCardIds,
    ...
    defineTool({
      ...
      description:
        '把全部关键帧生成结果提交给用户确认。**调用本工具后必须立即结束回合** —— 逐步确认模式下本工具会直接终止本回合（带 concludesTurn），此后视频生成/成片合成都会被门禁拒绝；不要读文件、不要加载 skill、不要做后续步骤的准备。用户在画布上方点「确认关键帧」后才继续；用户也可能点「打回重出」并按意见重出关键帧——那时你会收到带意见的重做指令，照它重出后**必须再次调用本工具**重新提交确认（用户仍有第二次裁决权）。用户可能直接在画布上对关键帧二次编辑（右键重试/修改提示词），编辑完成后仍需再次点击确认。放手跑模式（auto）直接放行，本工具是空操作。',
      parameters: {
    ...
    defineTool({
      ...
      description:
        '向用户提出一道点选题：选项卡片会内联显示在对话区（本工具调用卡片下方），用户点击后选择自动作为本工具结果返回（无需用户打字）。需求澄清阶段必须用本工具逐项提问（一次一个问题），不要用文本列表提问。列举类问题（如「需要调整哪些视觉细节？」）传 multiSelect=true 让用户勾选多项，答案以「、」拼接返回。问题会阻塞到用户作答或超时；超时返回提示时，采用带「推荐」标记的选项继续。放手跑模式（auto）下本工具**不提问、不等待**：直接返回按默认值取的答案与本次制作的锁定规格（画幅/时长/镜头数/分辨率），此时不要为了「等用户确认」反复调用它 —— 按返回的规格一路跑完即可。',
      parameters: {
      ...
      async execute(args, exec) {
        ...
        if (autoAnswer !== null) return { text: autoAnswer }
        const pending = {
          id: randomUUID(),
          question: a.question.trim(),
          options,
