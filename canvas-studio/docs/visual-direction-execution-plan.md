...
> | `scripts/preview-tray.mjs` | 托盘层叠验收台（CV-183），明暗双轨 13 条断言。用 `elementFromPoint` 问「成员图上这一点最上面是谁」——静态守卫锁不住层叠结果（DOM 顺序反了 / 被更高特异度压过 / 卡身改成半透明，源码里全都「看着对」）。页内**内置一组未修复对照**（场景 B：有 `csNodePrimary`、无 `csNodeTray`），**它必须红** —— 否则说明台子失去分辨力，比断言失败更危险 |
> | `scripts/preview-arrange.mjs` | 整理布局验收台（CV-185），明暗双轨 16 条断言。两侧都用**真实实现**摆（样式取 `styles.ts`、令牌取 `lib/brand.js`、坐标取 `computeArrangeLayout`、视图取 `computeFitView`），按**屏幕上的矩形**断言（不重叠 / 深度顺序 / 每张卡更大 / 不溢出舞台）。页内自带「整理前」对照 —— 把「整理后」换成「整理前」时 8 条断言当场红（实测），这就是台子的分辨力 |
...
> + 既有预览的令牌探针 = 12 组，当前 **23 组全绿**（含两个**负面组**「未知预设」与「镜头条旧写法」—— 它们必须失败，那是台子的分辨力自证）。
>
