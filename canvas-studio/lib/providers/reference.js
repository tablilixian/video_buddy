...
export async function readLocalAssetBytes(registry, projectId, file) {
 ...
 */
async function downsampleWithFfmpeg(asset, options) {
    let tempDir;
    try {
        ...
        const result = await runFfmpeg(ffmpeg, [
            ...
            String(FAL_JPEG_QV),
            outputPath,
        ], FFMPEG_TIMEOUT_MS, options.signal);
 ...
 */
export async function toFalDataUri(asset, options = {}) {
 ...
 */
export function assertFalReferenceSizes(dataUris) {
    let total = 0;
    for (const [index, uri] of dataUris.entries()) {
        ...
        if (size > FAL_MAX_SINGLE_REFERENCE_BYTES) {
            ...
            throw new Error(`参考图 ${index + 1} 编码后 ${mb}MB，超过 fal 单张上限 `
    ...
    if (total > FAL_MAX_TOTAL_REFERENCE_BYTES) {
        ...
        throw new Error(`参考图合计 ${mb}MB，超过 fal 单次请求上限 `
