...
export function coverInitial(name: string): string {
  ...
  // 逐字取第一个码点：noUncheckedIndexedAccess 下 `[0]` 的类型带 undefined，
  // 显式收口而不是断言 —— 名字非空在这里已经判过，undefined 分支不可达但仍要类型安全。
 ...
 */
export function coverTone(projectId: string): number {
  let hash = 0x811c9dc5
  for (let i = 0; i < projectId.length; i += 1) {
    hash ^= projectId.charCodeAt(i)
    hash = Math.imul(hash, 0x01000193)
  }
  return ((hash >>> 0) % COVER_TONES) + 1
}

/** 色档 → 样式类名（1..6 越界时夹回范围，脏数据不至于渲染成无色块）。 */
export function coverToneClass(projectId: string): string {
  const tone = coverTone(projectId)
  return `csCoverTone${tone}`
}
