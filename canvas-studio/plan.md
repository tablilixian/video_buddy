...
6. 设置弹窗出现 **输出 / 工作流 / 存储** 三个新标签页：
   - **输出 → 默认画幅比例**：改 `9:16` → 触发一次生成（文生图/视频）且 agent 未指定 aspectRatio 时，产物应为竖屏（按当前默认分辨率档 768p：1376×768 → 768×1376）；改回 `16:9` 恢复横屏。
   - **输出 → 默认分辨率**（CV-187）：档位 `480p` / `768p`（默认）/ `2k` 对应 16:9 的 864×480 / 1376×768 / 1920×1088（**图片与视频共用同一档位**）。改档后触发一次生成，产物像素应随之变化，且详情面板显示的分辨率与 `ffprobe` 实测一致。注意「导出质量（待接入）」是**成片码率**，与生成分辨率无关。
