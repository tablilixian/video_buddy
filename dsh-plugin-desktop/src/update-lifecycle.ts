...
function updateAvailableNotification(locale: DesktopLocale, version: string): DesktopNotification {
  return locale === 'zh'
    ? { title: 'VideoBuddy 有可用更新', body: `版本 ${version} 已可下载。打开 VideoBuddy 即可继续。` }
    : { title: 'VideoBuddy Update Available', body: `Version ${version} is ready to download. Open VideoBuddy to continue.` }
}
