 ...
 */
export function headTitleOf(node, label) {
 ...
 */
export const READING_ORDER = ['audio-mix', 'duration', 'dims', 'declared-duration', 'chars'];
/**
 * 节点**自带**的读数（不依赖媒体加载）。
 *
 ...
 */
export function declaredReadingsOf(node) {
    const out = [];
    ...
    if (node.kind === 'text' || node.kind === 'sticky' || node.kind === 'prompt') {
        ...
        if (chars > 0)
            out.push({ key: 'chars', text: `${chars} 字` });
    }
    return out;
}
