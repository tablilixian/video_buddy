...
- **`resolution`（`480p` / `736p` / `2k`，CV-187 起三档）**：档位 → 像素（16:9 基准）= `480p` 864×480 / `736p` **1280×736（默认）** / `2k` 1920×1088，竖屏反宽高、`1:1` 三档共用 1024×1024，宽高均为 32 的倍数。**图片与视频共用同一个档位**（`image_generate` 也有该参数）。**生效范围：仅 `fal` 供应商按档生效**（直通 480P/736P/2K）；`drama` 供应商暂不消费该档位（固定 0.4 MP ≈ 864×480），显式传入会回「暂未接入，已忽略」提示。不传则走设置页「默认分辨率」。**不要为分辨率向用户提问**（除非用户明确要求指定）；草稿/试拍可主动用 `480p` 提速（实测比 2k 快约 6 倍）。⚠️ 首帧图生视频（fal i2v）**画幅跟随首帧图**（该端点无画幅参数），故首帧图要按目标画幅出好。
