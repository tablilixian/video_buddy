...
/** 六段展示名（顺序即阶段序）。 */
export const WORKFLOW_STAGE_LABELS = ['剧本', '分镜', '定妆', '关键帧', '镜头', '成片'];
 ...
 */
export function stageOfNode(node) {
    if (isComposeProduct(node))
        return STAGE_FILM;
    if (node.toolName === BRIEF_NODE_TOOL)
        return STAGE_SCRIPT;
 ...
 */
export function productLabelOf(node) {
    if (isComposeProduct(node))
        return WORKFLOW_STAGE_LABELS[STAGE_FILM];
    if (node.toolName === BRIEF_NODE_TOOL)
        return WORKFLOW_STAGE_LABELS[STAGE_SCRIPT];
    if (node.toolName === STORYBOARD_NODE_TOOL)
        return WORKFLOW_STAGE_LABELS[STAGE_STORYBOARD];
    const byOperation = node.operationType === undefined ? undefined : OPERATION_PRODUCT[node.operationType];
    if (byOperation !== undefined)
        return byOperation;
    if (node.kind === 'audio')
        return KIND_PRODUCT.audio ?? null;
    if (node.kind === 'video')
        return '片段';
    if (node.isReference === true)
        return '参考';
    if (node.operationType === 'import')
        return '导入';
 ...
 */
export function deriveWorkflowStage(state, nodes) {
 ...
 */
export function workflowStateStageLabel(state) {
    const index = state === undefined ? STAGE_SCRIPT : (STATE_FLOOR[state] ?? STAGE_SCRIPT);
