...
| **打回（🔴 待修复）** | **CV-071** 详情弹窗未展示 GIF（hover 浮层正常，弹窗层缺 `<img>` 渲染；8 个 GIF 均就位，拟立项 **CV-112**）；**CV-073** 「我的 Skill」视角疑似未完成 + 点击后「全部技能」分区消失仅可返回主页（侧栏路由/卸载链路缺陷，拟立项 **CV-113**） |
...
| 悬停浮层 + 查看详情弹窗 | CV-071 | 🔴 待修复（弹窗未显示 GIF） | hover 浮层 GIF 正常；**详情弹窗层缺 `<img>` 渲染**（拟立项 CV-112） |
...
| 项 | 状态 | 依赖 / 阻塞 | 建议 |
|---|---|---|---|
| **CV-112 详情弹窗 GIF 缺失**（CV-071 打回） | 🔴 待修复 | SkillMarket 详情弹窗 `<img>` 渲染 | 弹窗层补 demo GIF 渲染（8 资源已就位） |
...
4. **打回两项**：CV-112（弹窗 GIF）、CV-113（我的 Skill 视图）单独立项修复。
...
> 本清单随验收进度更新；CV 编号推进见 STATUS.md（已编到 CV-111，新条目从 **CV-112** 起）。
