 ...
 */
import { PLAN_ASPECT_RATIOS, MAX_TARGET_DURATION, suggestShotCount, } from './contracts/project.js';
...
export const FALLBACK_RESOLUTION = DEFAULT_RESOLUTION;
/** 合法画幅判定 —— 复用契约里的 `PLAN_ASPECT_RATIOS`，不另抄一份字面量。 */
function isPlanAspectRatio(value) {
    return value !== undefined && PLAN_ASPECT_RATIOS.includes(value);
}
/**
...
export function resolveStudioDefaults(input = {}) {
    ...
    const aspectRatio = isPlanAspectRatio(plannedAspect)
        ? plannedAspect
        : isPlanAspectRatio(settingsAspect) ? settingsAspect : FALLBACK_ASPECT_RATIO;
    const aspectRatioSource = isPlanAspectRatio(plannedAspect)
        ? 'plan'
    ...
    const resolution = isVideoResolution(settingsResolution)
        ? settingsResolution
        : FALLBACK_RESOLUTION;
    return {
        aspectRatio,
 ...
 */
export function describeStudioDefaults(defaults) {
 ...
 * 「超时」与「放手跑」两条路上给出不同答案。
 */
export function recommendedOptionOf(options) {
 ...
 */
export function autoAnswerFor(input) {
    if (input.mode !== 'auto')
        return null;
