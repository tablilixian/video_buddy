...
export class ProjectRegistry {
     ...
     */
    async writeCanvasDocument(projectId, document) {
     ...
     * @param projectId - target project id.
     */
    async readActiveSkills(projectId) {
        let text;
        try {
     ...
     */
    async writeActiveSkills(projectId, skills) {
        const clean = [...new Set(skills.filter((entry) => typeof entry === 'string'))];
     ...
     */
    async releaseAssetNodes(projectId, assetId, keepNodeIds) {
        const keep = new Set(keepNodeIds);
        const existing = await this.readCanvas(projectId);
        const nodes = existing.nodes.map((node) => {
            if (node.assetId !== assetId || keep.has(node.id) === true)
                return node;
