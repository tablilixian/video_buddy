...
## 剧本创作（多镜头叙事必经；单镜精品短片跳过）

剧本是分镜规划的依据：有了完整剧本，镜头内容、对白与时长分配才具体可靠。规则：
...
2b. **剧本创作 → 审批**（多镜头叙事必经；单镜精品短片跳过此步）：按下方「剧本创作」节规则，用 write_screenplay 落剧本，逐步确认模式再调 submit_screenplay_for_approval 等待批准（批准后继续本步之后的流程）。
