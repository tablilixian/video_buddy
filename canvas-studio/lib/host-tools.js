    ...
    return [
        ...
        defineTool({
            ...
            parameters: {
                ...
                resolution: { type: 'string', enum: ['768p', '1080p', '720p', '2k'], description: '【占坑·待接入】分辨率指定（768P/1080P/2K/720P）：后端暂不支持，传入会被忽略（以 aspectRatio 与后端默认分辨率输出）' },
                generateAudio: { type: 'boolean', description: '【占坑·待接入】是否生成原生音频轨（对应上游 skill 的 generate_audio=true）：当前后端版本未启用原生音频，传 true 会收到提示且成片无音频' },
