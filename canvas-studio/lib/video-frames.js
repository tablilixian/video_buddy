 ...
 * `时长 - ε`（两位小数，不低于 0）。
 */
export function planLastFrameSeek(durationSec) {
 ...
 */
export async function extractLastFrame(registry, projectId, videoUrl, options = {}, signal) {
    ...
    if (!existsSync(inputPath)) {
        throw new Error(`视频文件不存在（${videoUrl}）。请确认传入的是本项目画布上视频节点的 url（video_generate / video_composite 工具返回的 url 字段）。`);
    }
    ...
    const extraction = await runFfmpeg(ffmpegPath, ['-ss', planLastFrameSeek(duration).toFixed(2), '-i', inputPath, '-frames:v', '1', '-q:v', '2', '-y', framePath], FFMPEG_TIMEOUT_MS, signal);
