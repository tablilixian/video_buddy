 ...
 */
export async function healReferenceFilename(
  registry: ProjectRegistry,
  projectId: string,
  filename: string,
  signal?: AbortSignal,
...
export async function analyzeImage(
  ...
  const call = async (image: string): Promise<string> => {
...
export async function generateAsset(
  ...
  const result: GenerateResult = { url, width: mediaSize.width, height: mediaSize.height }
...
export async function generateCharacterSheet(
  ...
  const sourceIds = mergeSourceIds(
    resolveSourceIds(canvas.nodes, params.sourceUrls),
    resolveSourceIdsByFilename(canvas.nodes, [params.filename]),
  )
