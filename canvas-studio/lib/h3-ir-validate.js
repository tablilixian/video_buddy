...
const ALIGNED_MODES = ['I2VA', 'L2VA', 'FL2VA'];
...
// K1-K3 — 对齐行，逐字匹配
const ALIGN_I2VA = /^For the target video, at 0\.00 seconds into the target video, <Picture 1> \(from \[Shot 1\]\) is fully referenced\.$/;
const ALIGN_FL2VA = /^How the reference pictures align with the target video — Picture 1 \(from Shot 1\) aligns with the 0\.00-second mark of the target video; Picture 2 \(from Shot (\d+)\) aligns with the (\d+\.\d{2})-second mark of the target video\.$/;
const ALIGN_L2VA = /^How the reference pictures align with the target video — <Picture 1> \(from \[Shot (\d+)\]\) aligns with the (\d+\.\d{2})-second mark of the target video\.$/;
...
export function validateH3Ir(text, opts) {
    ...
    const findings = [];
    const err = (rule, message) => findings.push({ rule, severity: 'ERROR', message });
    const warn = (rule, message) => findings.push({ rule, severity: 'WARN', message });
    const isRef = mode === 'Ref2VA';
    const names = isRef ? REF_SECTIONS : BASE_SECTIONS;
    const other = isRef ? BASE_SECTIONS : REF_SECTIONS;
    ...
    if (!startsOk)
        err('S6', `第一行既不是段名也不是对齐行: ${q(firstLine)}`);
    // ---------- S1/S2 段名与顺序 ----------
    ...
    if (missing.length)
        err(isRef ? 'S2' : 'S1', `缺少段: ${missing.join(', ')}`);
    ...
    for (const m of hits) {
        ...
        if (isRef && inline)
            warn('S4', `六段式的 ${m.name} 正文通常另起一行`);
        if (!isRef && !inline)
            warn('S3', `三段式的 ${m.name} 正文通常与段名同一行`);
    }
    // ---------- S5 段间分隔 ----------
    ...
    if (!nums.length) {
        ...
        if (nums[0] !== 1)
            err('T5', `第一个镜头应是 [Shot 1]，实际是 [Shot ${nums[0]}]`);
    ...
    for (const m of shotIter) {
        ...
        tsSecs.push([n, t]);
        if (t >= duration)
            err('T4', `[Shot ${n}] 时间戳 ${t.toFixed(3)}s 不小于目标时长 ${duration}s`);
        const after = tail.slice(tm[0].length).replace(/^\s+/, '');
        const afterLower = after.toLowerCase();
        const startsWithVerb = CUT_VERBS.some((v) => afterLower.startsWith(v) || afterLower.startsWith(v.split('the ', 2)[1] ?? v));
        if (!startsWithVerb) {
    ...
    for (let i = 1; i < tsSecs.length; i++) {
        ...
        if (tb <= ta)
            err('T3', `时间戳未严格递增: [Shot ${na}]=${ta.toFixed(3)} → [Shot ${nb}]=${tb.toFixed(3)}`);
    }
    // ---------- T7 转场 ----------
    ...
    if (nds && nds.trim() !== 'N/A') {
        ...
        for (const w of ['melancholic', 'hopeful', 'uplifting', 'emotional', 'nostalgic',
            ...
            if (ndsLower.includes(w))
                warn('L3', `non_diegetic_music 含抽象情绪词 '${w}'`);
        }
    }
    // ---------- L1 词数（仅 WARN） ----------
    if (main) {
    ...
    // ---------- 素材标签编号 ----------
    const labelNums = (kind) => [...new Set([...text.matchAll(new RegExp(`<${kind} (\\d+)>`, 'g'))].map((m) => parseInt(m[1], 10)))].sort((a, b) => a - b);
    ...
    function validateRef() {
        ...
        if (!m) {
            ...
            if (bad.length)
                err('R10', `非法任务类型 ${bad.join(', ')}；合法值 ${TASK_TYPES.join(', ')}`);
            if (new Set(parts).size !== parts.length)
                err('R11', `任务类型重复: ${parts.join(', ')}`);
            if (!m[1].includes(' + ') && parts.length > 1)
                err('R11', '多个任务类型必须用 ` + ` 连接');
        ...
        const declared = new Set([...defs.matchAll(/<(Subject|Picture|Video|Audio) (\d+)>/g)].map((x) => `<${x[1]} ${x[2]}>`));
        const tracked = new Set([...defs.matchAll(/^(<(?:Subject|Picture|Video|Audio) \d+>)/gm)].map((x) => x[1]));
        if (!tracked.size)
            err('R3', 'subject_definitions 里没有任何自成一行的引用标签定义');
        // R14 summary 不得引入新标签
        ...
        if (newInSummary.length)
            err('R14', `summary 引入了未定义的新标签: ${newInSummary.join(', ')}`);
        // 使用前必须已定义（检查 detailed_description）
        const used = new Set((sections.get('detailed_description') ?? '').match(/<(?:Subject|Picture|Video|Audio) \d+>/g) ?? []);
        const undefinedUsed = [...used].filter((t) => !declared.has(t)).sort();
        if (undefinedUsed.length)
            err('R2', `detailed_description 使用了未定义的标签: ${undefinedUsed.join(', ')}`);
        // R16-R18 retention
        ...
        for (const line of retLines) {
            ...
            const [, label, kind, , marker, reason] = lm;
            covered.add(label);
            const allowed = kind === 'Audio' ? AUDIO_RETENTION : VISUAL_RETENTION;
            if (!allowed.includes(marker)) {
                err(kind === 'Audio' ? 'R18' : 'R17', `${label} 的档位 '${marker}' 不在${kind === 'Audio' ? '音频' : '视觉'}合法集内 ${allowed.join(', ')}`);
            }
            if (!reason.trim())
                err('R19', `${label} 缺少理由`);
        }
        ...
        if (uncovered.length)
            err('R16', `retention_analysis 未覆盖单独追踪的标签: ${uncovered.join(', ')}`);
        const extra = [...covered].filter((t) => !declared.has(t)).sort();
        if (extra.length)
            err('R16', `retention_analysis 出现未定义标签: ${extra.join(', ')}`);
        // R22 (Sx) 禁入 retention
        if (/\(S\d/.test(retention))
            err('R22', 'retention_analysis 里出现了 (Sx)，明文禁止');
    }
 ...
 * 仍带有多个段名或标签，能被拦下。
 */
export function looksLikeH3Ir(text) {
    const seen = new Set();
    for (const marker of IR_MARKERS) {
        if (marker.re.test(text))
            seen.add(`${marker.kind}:${marker.re.source}`);
        if (seen.size >= 2)
            return true;
    }
    return false;
...
const ALIGN_I2VA_TEXT = 'For the target video, at 0.00 seconds into the target video, <Picture 1> (from [Shot 1]) is fully referenced.';
/**
 ...
 */
export function prepareH3IrPrompt(text, opts) {
    const repairs = [];
    ...
    if (trimmed !== out) {
        out = trimmed;
        if (!repairs.some((r) => r.includes('S6')))
            repairs.push('去除首尾多余空白（S6）');
    }
