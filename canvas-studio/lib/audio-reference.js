...
/** 官方支持的音频容器（小写扩展名，含点）。 */
export const H3_AUDIO_EXTENSIONS = ['.mp3', '.wav'];
/** 取小写扩展名（含点）；无扩展名返回空串。 */
export function audioExtensionOf(filename) {
 ...
 */
export function validateH3AudioReferences(refs, visualCount) {
    const issues = [];
    if (refs.length === 0)
        return issues;
 ...
 */
export function audioModeNotice(baseCapability, visualCount) {
    if (baseCapability !== 'first-last-frame')
        return undefined;
