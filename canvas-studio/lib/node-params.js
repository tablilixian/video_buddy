 ...
 */
export function parseGenerationParams(raw) {
...
export function generationParamsOf(node) {
 ...
 */
export function isReplayable(node) {
    if (node.toolName === undefined)
        return false;
    if (!REPLAYABLE_TOOLS.includes(node.toolName))
        return false;
    return generationParamsOf(node) !== null;
}
const PROMPT_ONLY = [{ key: 'prompt', label: '提示词' }];
/**
 * toolName → 可编辑的自由文本字段。
 *
 ...
 */
export function promptFieldsOf(node) {
    if (node.toolName !== undefined) {
    ...
    if (params !== null && typeof params.prompt === 'string')
        return PROMPT_ONLY;
    return [];
}
/** 读某个提示词字段的当前值（缺省 / 非字符串一律空串）。 */
export function promptValueOf(node, key) {
    const params = generationParamsOf(node);
 ...
 */
export function withPromptField(raw, key, value) {
