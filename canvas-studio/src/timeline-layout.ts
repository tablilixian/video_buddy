...
const TICK_STEPS = [1, 2, 5, 10, 15, 30, 60] as const

/**
 * 标尺右端点：总时长向上取整到刻度步长的整数倍。
 * 例：total=10.7 → step=2 → rulerMax=12（刻度 0,2,4,…,12）。
 */
export function niceRulerMax(totalSeconds: number): number {
 ...
 */
export function rulerTicks(rulerMax: number): readonly { t: number, pct: number }[] {
 ...
 */
export function planClipLayout(clips: readonly TimelineClipInput[]): readonly ClipSpan[] {
  const spans = clips.map(clip => {
    const measured = typeof clip.duration === 'number' && Number.isFinite(clip.duration) && clip.duration > 0
    return { id: clip.id, span: measured ? (clip.duration as number) : FALLBACK_CLIP_S, measured }
  })
...
export function clipTotalSeconds(clips: readonly TimelineClipInput[]): number {
  return clips.reduce((sum, clip) => {
    const measured = typeof clip.duration === 'number' && Number.isFinite(clip.duration) && clip.duration > 0
    return sum + (measured ? (clip.duration as number) : FALLBACK_CLIP_S)
  }, 0)
}
 ...
 */
export function playheadPx(progress: number, usableWidth: number): number {
  const p = Math.min(1, Math.max(0, progress))
  return p * usableWidth
}

/** 播放头时间 → 相对 rulerMax 的百分比（0-100，越界夹紧）。 */
export function playheadLeftPct(timeSeconds: number, rulerMax: number): number {
...
export function clipIdAt(timeSeconds: number, spans: readonly ClipSpan[]): string | undefined {
  for (const span of spans) {
    if (timeSeconds >= span.start && timeSeconds < span.start + span.span) return span.id
  }
  return undefined
}
