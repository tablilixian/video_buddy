...
> **状态**：**DD-00 / DD-01 / DD-02 / DD-03 已落地**（CV-161 前两批 + CV-162 第三批）；
> DD-04/05/06 仍待逐批开工。
> **编号**：DD-00 起（`DD` 独立序列，不占 `CV-xxx`）。CV 条目已立：**CV-161**（DD-01 + DD-02）、
> **CV-162**（DD-03）；下一批立案从 `CV-163` 起。
>
> **落地记录（2026-09-12）**：
>
> | 批次 | 状态 | 落地内容 | 验证 |
...
> | DD-04a | ⬜ 待开工 | 时间轴骨架（时间标尺 / 播放头 / 真实时长比例 / 多轨） | — |
> | DD-04b | ⬜ 待开工 | 真波形（Host ffmpeg 抽包络 + 抽根级纯函数） | — |
> | DD-05 | ⬜ 待开工 | 场记板审批条 + 六阶段轨道 + 动效三语义归位 | — |
> | DD-06 | ⬜ 待开工 | 首屏与三态（Lobby / Welcome） | — |
>
> **落地时才拿到的实测发现（回填 §0）**：
