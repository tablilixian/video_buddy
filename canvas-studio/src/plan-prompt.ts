...
function agentCwdOf(context: unknown): string | undefined {
 ...
 */
export function planSectionText(plan: StudioProjectPlan | undefined): string {
...
async function projectOf(registry: ProjectRegistry, cwd: string) {
 ...
 * @returns 是否存在有效预置文本。
 */
export async function refreshPlanPromptCache(registry: ProjectRegistry, cwd: string | undefined): Promise<boolean> {
  if (!cwd) return false
  try {
...
/** 读取缓存文本（未预热返回空串）。 */
export function cachedPlanSectionText(cwd: string | undefined): string {
  if (!cwd) return ''
 ...
 */
export function registerProjectPlanPrompt(ctx: {
