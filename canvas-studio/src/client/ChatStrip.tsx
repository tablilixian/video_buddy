...
import { WORKFLOW_STAGE_LABELS } from '../workflow-stage.js'
...
export function ChatStrip(props: ChatStripProps): ReactElement {
  const { stageIndex, onExpand } = props
  return (
    <div className="csChatStrip">
      <button
        type="button"
      ...
      >
        {WORKFLOW_STAGE_LABELS.map((label, i) => (
          <span
            key={label}
            className={'csChatStripDot'
              + (i === stageIndex ? ' csChatStripDotNow' : i < stageIndex ? ' csChatStripDotDone' : '')}
          />
        ))}
      </div>
    </div>
