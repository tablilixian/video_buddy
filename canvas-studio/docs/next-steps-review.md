...
### 批次 2 — 交互补全（约 1–2 天）—— 首批 5 项 ✅ 已实施（2026-08-29，未提交）

1. ~~**CV-015** toast 替换 alert~~ —— 7 处 `window.alert` 全部换为底部居中 toast（普通 3.5s / 错误 6s 自动消失）；成片成功 toast 不带「定位到节点」动作（F1 已自动居中，冗余）
2. ~~**CV-016** 右键空白菜单~~ —— 新组件 `CanvasBlankMenu`：在此新建便签/文本/提示（落光标处，`addNode` 加可选 `at`）+ 粘贴 + 适配视野；与节点菜单关闭语义一致、互斥打开
3. ~~**CV-017** 方向键微调~~ —— 1px / Shift 10px，锁定节点跳过；800ms 连发只入一条 undo 快照；`computeNudge` 纯函数（+3 测试例）
4. ~~**CV-019** 缩放到选中~~ —— Handle 加 `zoomToSelection`（无选中退化 fitToContent）+ 双击空白 fit；fit 数学抽 `fitToBounds` 共用
5. ~~**CV-011** 参考角标 + 空态引导~~ —— 参考节点左下「参考 · 角色」色点角标（四角色四色）；托盘空态渲染引导卡片

**CV-008 多选拖拽 + group 联动 + 框选未做** —— gesture 结构改动较大（Gesture 单 nodeId → id 集合 + group 递归带动 + marquee 状态机），按约定先给方案讨论再动手。

**验收方法（与批次 0/1 同一次重建）**：
1. 造一次上传失败 / 成片导出 → 底部应出现非阻塞 toast 并自动消失（不再弹系统 alert）
2. 右键画布空白处 → 菜单出现；「在此新建便签」应落在光标处；Escape / 点其他处关闭
3. 选中节点按方向键 → 1px 微调；Shift+方向 → 10px；连续快按后 Cmd+Z 一次回到微调前
4. 选中若干节点（Cmd+A 或 Ctrl 点选）→ 双击空白应适配全部视野；工具栏若接 zoomToSelection 则只缩放到选中
5. 上传一张图并标记参考 → 节点左下出现带色点的「参考 · 角色」角标；无参考时画布左上应显示空态引导卡
...
| # | 议题 | 选项 |
| --- | --- | --- |
| 1 | **下一步做什么**（17:40 更新：批次 0 **已提交 `6b3091a772`**、批次 1 **已提交 `5eb97acb5b`**、批次 2 首批 5 项 **已实施未提交**） | 桌面回归批次 0–2（同一次重建）后 → **CV-008 方案讨论** / 批次 3 工作流硬伤（O1 + CV-036，需拍板） |
