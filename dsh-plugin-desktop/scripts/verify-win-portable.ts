...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `VideoBuddy-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'VideoBuddy.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing VideoBuddy.exe: ${portablePath}`)
  }
