...
/** 把任意条数请求收口到合法区间。 */
export function clampWaveBars(bars: number): number {
 ...
 */
export function waveBarsDeterministic(seedText: string, bars: number): number[] {
 ...
 */
export function waveBarsFromEnvelope(envelope: readonly number[], bars: number): number[] {
  const count = clampWaveBars(bars)
