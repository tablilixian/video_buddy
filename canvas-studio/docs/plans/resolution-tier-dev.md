...
**残余缺陷（本轮不修，已单列）**：Drama 视频真实产物恒为 **864×480**（0.4 MP），而节点按档位落盘 `mediaWidth/mediaHeight`（默认档 = 1376×768）→ §1.2 那条「声明 ≠ 真实」在 **Drama 视频**上依然存在。

> 修它有两条路，都要等 P0-d 复测：**①** 后端认 1.0 ⇒ drama 按档发 megapixels（像素自然对齐）；**②** 后端不认 ⇒ drama 侧启用「实际档位恒为 480p」的诚实声明（`OUTPUT_SIZE['480p']`）。**两条路都必须先有实测**，否则只是把「错的数字」换成「另一个错的数字」。
>
...
| # | 场景 | 期望 |
|:--|:--|:--|
| 1 | **老节点重试**（画布上 fal 视频节点，`generationPrompt` 带 `720p`）→ 右键重试 | **不崩**，正常出片，无多余 warning |
| 2 | 显式传 `resolution: '480p'` 出视频 | 出片，详情面板分辨率显示 **864 × 480** |
| 3 | 不传档位出视频 | 按设置档（默认 768p）→ 详情面板 **1376 × 768** |
| 4 | 不传档位出图 | 1376×768（**不再是 1280×720**） |
| 5 | 竖屏 + `2k` | 1088 × 1920（反宽高生效） |
| 6 | 1:1 出图 + 三档各一次 | 三档都是 1024 × 1024 |
| 7 | 详情面板分辨率 vs `ffprobe` 真实产物 | **必须一致**（这是 §1.2 缺陷的验收条件） |
