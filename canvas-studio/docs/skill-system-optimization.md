...
| 5 | ~~**B3** creative-mechanisms 按风格打通~~ | **CV-120** | — | — | ✅ 已落地·待验收：激活风格 skill 后 agent 写 IR 前查速查表并选用对应机制 |
| 6 | ~~**B2 + 总纲拆分**~~（拆分落地；去重走方案 Z+优先级反转，薄壳化退 backlog） | **CV-121** | — | — | ✅ 已落地·待验收：总纲 49KB→14.8KB 骨架+7 分册；风格冲突按风格 skill 走且不越安全底线 |
| 7 | **B4** L2VA 独立入口 | CV-122 | 需后端确认 | 中 | fal/drama 是否支持尾帧锚定 |
| 8 | **B5** 占位工具预期管理 | 归入 O 系列 | 产品决策 | — | 不在本文件跟踪 |

> 编号规则：`CV-112` / `CV-113` 已被「详情弹窗 GIF」「我的 Skill 视图」拟立项占用。**CV-116 ~ CV-121 已于 2026-09-09 全部正式登记到 STATUS.md**（对应 A1/A2/A3/B1/B3/B2 落地）；下一个可用号为 **CV-122**（B4 顺延占位不变）。
...
- [x] ~~A 组桌面验收~~（2026-09-09 用户确认：东方神话上广场 / 试跑角标 / 「暂无预览」降级均通过）
- [x] ~~确认 B2 风格 skill 去重的决心~~（2026-09-09 用户拍板：方案 Z + 优先级反转——冲突以风格 skill 为主，安全底线例外；CV-121 已落地）
- [x] ~~`plan.md` 加归档头~~（2026-09-09：已归档并指向 STATUS.md / 本文件）
- [ ] **桌面验收三连**（一次跑片可全覆盖）：CV-119 坏 IR 被当场拦（报错含 S1/M1/K5/T4 规则名）/ CV-120 激活风格后写 IR 前查机制速查表 / CV-121 按指针读分册 + 风格冲突按风格走且不越审批门禁
- [ ] **B4（CV-123）前置调研**：向 fal / drama 确认是否支持尾帧锚定，再决定 L2VA 独立入口做不做（CV-122 已被 character_sheet 去切分依赖占用）
