...
describe('published package surface', () => {
  ...
  it('fixes the installed application identity', () => {
    ...
    expect(manifest.build?.appId).toBe('com.videobuddy.desktop')
    ...
    expect(manifest.build?.win?.artifactName).toBe('VideoBuddy-${version}-${arch}-Portable.${ext}')
    expect(manifest.build?.nsis).toEqual({
      ...
      useZip: false,
      artifactName: 'VideoBuddy-${version}-${arch}-Setup.${ext}',
    })
  ...
  it('keeps the iOS Default source icon unmodified', () => {
    ...
    expect(digest).toBe('8383931022dbe92fe7a3bc4fec3ea8d218924a4de7caffb91a0db527aae255d5')
  })
