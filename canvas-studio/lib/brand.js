 ...
 */
export const BRAND_PRESET_IDS = ['cinema-violet', 'ocean-blue', 'ember-violet', 'amber-creative'];
export const DEFAULT_BRAND_PRESET = 'cinema-violet';
...
export function resolveBrandPreset(id) {
    if (id !== null && id !== undefined && id in BRAND_PRESETS)
...
const NON_COLOR_TOKENS = [
    ...
    // 景深（DD-02：拖动时按血缘距离分档，远亲与无关节点降到该不透明度）
    ['--cs-dim', '0.42'],
    // CV-186：中间档 —— 隔着**一层**血缘的节点（血缘距离 2）。DD-03 原版只有
    // 「亮 / 压暗」两档，实测在真画布上会把 68% 的卡片一次压到 0.42，读起来是
    // 整屏变灰；给 2 跳单独一档之后平均压暗降到 28%，且没有一个拖动目标超过 70%。
    ['--cs-dim-near', '0.75'],
 ...
 */
export function brandCssText(presetId) {
