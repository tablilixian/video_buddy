...
本 Skill **先按用户输入语言整理 prompt，并把 MiniMax H3 作为用户确认后的生成步骤（本项目视频模型固定为 MiniMax H3，无模型选择）**。用户明确确认 H3 生成前，不生成视频。确认必须通过 `ask_user_choice` 选项卡完成，不要用纯文本求确认。不要为单纯 prompt 写作转交 planner 或 executor。最终 prompt 的语言必须跟随用户输入的主语言：用户用中文就写中文，用户用英语就写英语，用户用日语就写日语；混合输入时使用占主导的语言；难以判断时使用当前对话语言。只有用户要求保留的专名、模型名或字面参数可以原样保留。
...
- 除非用户要求解释，最终回答先给与用户输入主语言一致的 prompt 文本，再附一句同语种的一句话说明：确认后将用 MiniMax H3 生成 15 秒 16:9 视频。
- 输出 prompt 后**必须调用 `ask_user_choice` 求确认**（问题同语种，选项如中文：`确认，用 H3 生成视频（推荐）`、`先调整 prompt`），不要用纯文本问句求确认，也不要结束回合等待用户打字回复。
- 用户通过选项卡明确确认 H3 生成前，不生成视频、图片、音频、分镜或中间资产。
- 视频模型固定为 MiniMax H3，不要提及或提供其他视频模型选项；只有用户主动要求时才把 Seedance 作为目标使用语境提及，且不要在创意 prompt 内添加模型参数。
...
先按要求输出与用户输入主语言一致的最终 prompt，随后**立即调用 `ask_user_choice`**（同语种问题与选项，推荐项放最前：`确认，用 H3 生成视频（推荐）`、`先调整 prompt`），让用户点选确认。不要包含标题、清单、文件名或画布交付说明。用户通过选项卡确认 H3 生成后，再生成视频并检查接触、连续变形、慢半拍追拍和非恐怖手绘质感是否成立；用户选择调整时，按其反馈修订 prompt 并重新走确认选项卡。
