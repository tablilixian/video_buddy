...
/** user/message 载荷里用于取正文的最小形态（ContentBlock 的 text 块）。 */
interface BriefMessageView {
  readonly id?: unknown
...
function briefTextOf(message: BriefMessageView): string {
 ...
 */
export function createBriefCaptureDefinition(hooks: BriefCaptureHooks) {
  return {
    kind: 'canvas-studio-brief',
    match(event: BriefCaptureEvent): BriefCaptureMatchResult | null {
      if (event.type !== 'user/message') return null
      const message = (event.data ?? {}) as BriefMessageView
      // 只有真人直接输入才算「创意」；合成注入一律忽略。
      if (message.source?.kind !== 'user') return null
      return { id: String(message.id ?? ''), role: 'start' }
    },
    start: (_context: unknown, startMatch: { readonly event: BriefCaptureEvent }): null => {
      const projectId = hooks.getSelectedProjectId()
      if (projectId !== null && !hooks.hasBriefNode(projectId)) {
        const text = briefTextOf((startMatch.event.data ?? {}) as BriefMessageView)
