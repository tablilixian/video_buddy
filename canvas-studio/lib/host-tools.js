...
const resultSchema = {
    ...
    properties: {
        ...
        width: { type: 'integer', description: '产物宽度（像素）。**视频是 ffmpeg 实测的真实产物像素**（与画布节点 mediaWidth 同源；探测不可用时才回退本次档位声明值）；图片侧后端按请求尺寸逐字节出图，两者相等。要判断实际出了多大分辨率，以此为准' },
        height: { type: 'integer', description: '产物高度（像素）。口径同 width' },
        ...
        warnings: { type: 'array', items: { type: 'string' }, description: '提示（可选）：本次未生效或被后端拒绝的参数说明（如 model 未接入），或产物探测失败时的回退说明。非空时必须告知用户，不要声称已按该参数生成' },
 ...
 */
const lookCardSchema = {
    type: 'object',
    additionalProperties: false,
    properties: {
        assetId: { type: 'string', description: '建立的 Look 卡 id' },
        name: { type: 'string', description: 'Look 卡显示名（已归一，含 `Look · ` 前缀）' },
        lockedPrompt: { type: 'string', description: '冻结后的 5 项 tokens（逐镜逐字节复用；补齐字段时同名重调本工具覆盖）' },
        anchors: { type: 'array', description: '锚点摘要（title / url / 后端产物名）；无锚点时为[]。要把它作参考图，请用 @ref[title]' },
        warnings: { type: 'array', items: { type: 'string' }, description: '非致命提示（tokens 未解析到全部 5 项 / 锚点没对应上画布节点）' },
    },
};
/**
...
function shotCardTitleOf(nodes, node) {
    for (const id of node.sourceIds) {
        ...
        if (found?.toolName === STORYBOARD_NODE_TOOL)
            return found.title;
    }
    return undefined;
}
...
/** 把 look_card 结果渲染成模型可读的文本块（含逐镜注入纪律与锚点用法）。 */
function renderLookCardResult(_args, value) {
    const v = value;
    const lines = [
        `已建立 Look 卡「${v.name}」（id=${v.assetId}）—— 全片视觉基调的权威来源。`,
        '冻结的 5 项 tokens（每镜 prompt 以它开头逐字节复用）：',
        v.lockedPrompt,
    ];
    const anchor = v.anchors[0];
    if (anchor !== undefined) {
        // CV-155 同一条纪律：锚点上的 filename 是**后端产物名**，作参考图必须走 @ref 换句柄。
...
/**
 * CV-157：把 Look 卡的锚点引用解析成画布节点 id。
 *
 * Agent 手里可能只有三种东西之一，三种都要认：`@ref[节点标题]`、画布节点 id、
 * `upload_image` 得到的文件名（以及本地资产名）。先走 `findNodeByRef`（id 精确 → 标题）；
 * 不中再走 `resolveRefValue` —— 对 `@ref` 它顺带把节点上的**产物名换成可用句柄并回写**，
 * 于是「解析后的名字」就等于节点上的 `filename`，可直接反查。查不到返回 null，
 * 由 `registerLookCard` 出一条可操作 warning（**不阻断落卡**：tokens 文字注入才是主路径）。
 */
async function resolveAnchorNodeId(registry, projectId, ref) {
    ...
    const localName = (url) => url.split('/').pop() ?? null;
    const hit = after.nodes.find((node) => node.filename === resolved)
        ?? after.nodes.find((node) => node.filename === ref)
        ?? after.nodes.find((node) => node.url !== undefined && localName(node.url) === ref);
    return hit?.id ?? null;
}
/**
 ...
 */
async function assertApprovalAllowed(registry, projectId, tool, shotBound) {
 ...
 */
const RESOLUTION_ENUM = Object.keys(OUTPUT_SIZE);
/**
 * 视频侧 resolution 参数描述。
 *
 ...
 */
const RESOLUTION_PARAM_DESC = '分辨率档位：480p(864×480) / 768p(1376×768，默认) / 2k(1920×1088)，宽高均为 32 的倍数（H3 规格）。'
 ...
 */
const IMAGE_RESOLUTION_PARAM_DESC = '输出分辨率档位：480p(864×480) / 768p(1376×768，默认) / 2k(1920×1088)，16:9 基准，'
...
export function mergeShotCards(existing, sourceIds, shots, mint) {
    ...
    shots.forEach((cells, index) => {
        ...
        if (hit !== undefined && no !== undefined) {
            ...
            const merged = { ...rest, title: shot.title, text: shot.text, sourceIds: [...sourceIds], ...durationFields };
            updated.push(merged);
            cards.push(merged);
    ...
    return [
        defineTool({
            ...
            parameters: {
                ...
                filenames: { type: 'array', description: '可选多参考图（最多 4 张，来自 upload_image 工具）；与 filename 二选一，多参考融合图生图' },
                replaces: { type: 'string', description: '可选：本次生成的图取代哪个已有图片节点（填节点 id，来自此前工具结果的 nodeId 或 list_references）。旧图自动标记失效并退出参考池；重出样张 / 重做参考图时应传，避免画布上堆废图' },
        ...
        defineTool({
            ...
            async execute(args, exec) {
                ...
                return registerLookCard(registry, projectId, {
                    ...
                    ...(anchorNodeId !== null
                        ? { anchorNodeId }
                        : a.referenceFilename !== undefined ? { anchorRef: a.referenceFilename } : {}),
                    ...(a.negativePrompt !== undefined ? { negativePrompt: a.negativePrompt } : {}),
                });
            },
        }),
        ...
        defineTool({
            name: 'qc_shot',
            description: '对单个镜头产物做**一致性质检**：视觉模型对照固定要素描述核对画面（外貌/发型发色/服装/核心道具），基准里带 Look 卡时还逐项核对风格维度（色彩/光线/材质/镜头语汇；「节奏」单帧不可判，不参与判定），返回 PASS / FAIL / WARN 与漂移项，结论写回该画布节点。每镜出图后调一次；FAIL 只重跑该镜（同一 shotRefs），且必须**修复式重跑**——把漂移项转成纠正指令追加到 prompt 再重出，不要原样重跑（原样重跑只换种子，同样的漂移会重现）。判定基准缺省自动取本项目全部一致性资产卡的 lockedPrompt（**角色/场景卡按逐项一致、Look 风格卡按整体调性分组判定**，可先调 list_references 查看），也可显式传 expect。WARN=判定不明确，不要自动重跑、也不要中途打断用户——记录下来回合末统一汇总。⚠️ **放手跑（auto）模式下本工具被 Host 自动跳过**（返回 skipped，不调用视觉模型）——一致性由锁定提示词逐字节注入与参考图锚点保障；需要质检请切换逐步确认模式。',
            parameters: {
            ...
            output: {
                schema: {
                    ...
                    properties: {
                        ...
                        skipped: { type: 'boolean', description: 'true=放手跑模式已跳过本次质检（未调用视觉模型，见 reason）' },
                ...
                render: (_args, value) => {
                    ...
                    if (v.skipped === true)
                        return [{ type: 'text', text: v.reason ?? QC_AUTO_MODE_NOTICE }];
        ...
        defineTool({
            ...
            parameters: {
                includeRetired: { type: 'boolean', description: '可选：是否一并列出已失效（被取代 / 作废）的参考图（默认 false，只列有效参考）' },
            },
            output: {
                schema: {
                    type: 'object',
        ...
        defineTool({
            ...
            parameters: {
                ...
                filename: { type: 'string', description: '可选：首帧图的**句柄**（upload_image 返回，或 @ref[显示名]——Host 会把画布节点上的产物名自动换成句柄），用作视频首帧；不传则为纯文生视频。**本工具只有这一个图片位次**——它就是 `<Picture 1>`（首帧）；要首尾帧用 video_composite 传 2 张。⚠️ 生成工具结果里的产物名（形如 img_01287_.png）不能直接传' },
                ...
                resolution: { type: 'string', enum: RESOLUTION_ENUM, description: RESOLUTION_PARAM_DESC },
                ...
                irMode: { type: 'string', enum: ['T2VA', 'I2VA', 'FL2VA', 'Ref2VA'], description: '可选：本镜 H3-Context-IR 简报的**显式模式声明**。写 IR 时建议声明 —— 预检先核对声明与素材位次是否一致（' + COUNT_MODE_HINT + '），不一致**立即**报「模式声明不一致」（而不是一堆段名/对齐行 ERROR）；一致则按声明模式校验。纯文本提示词忽略本参数。⚠️ 声明不能改变端点路由：端点由素材数量决定，「风格参考 + 首帧」只能两步走' },
            },
            output: { schema: resultSchema, render: renderResult },
            async execute(args, exec) {
                const a = args;
        ...
        defineTool({
            ...
            parameters: {
                ...
                filenames: { type: 'array', required: true, description: '参考图的**句柄**数组（upload_image 返回，或 @ref[显示名]——Host 会把画布节点上的产物名自动换成句柄）。⚠️ 生成工具结果里的产物名不能直接传。**顺序即语义与位次**：1 张=首帧（I2VA）；2 张=首帧+尾帧（FL2VA，第 1 张首帧、第 2 张尾帧）；≥3 张=多参考合成（Ref2VA，第 N 张即 `<Picture N>`）。上限由供应商决定：Drama 6 张、fal 9 张，超出自动采样（保留首尾）' },
                aspectRatio: { type: 'string', enum: ['16:9', '9:16'], description: '宽高比，默认 16:9。视频只有横屏 16:9 与竖屏 9:16 两档' },
                ...
                replaces: { type: 'string', description: '可选：本次生成取代哪个已有视频节点（填其画布节点 id，用 list_shots 查）。用于「改了关键帧重出这一镜」——旧版自动失效、不再进默认合成。同关键帧同参数重复生成会自动取代，无需显式传' },
                irMode: { type: 'string', enum: ['T2VA', 'I2VA', 'FL2VA', 'Ref2VA'], description: '可选：本镜 H3-Context-IR 简报的**显式模式声明**。写 IR 时建议声明 —— 预检先核对声明与素材位次是否一致（' + COUNT_MODE_HINT + '），不一致**立即**报「模式声明不一致」；一致则按声明模式校验。纯文本提示词忽略本参数。⚠️ 声明不能改变端点路由（2 张图仍走首尾帧端点）：2 张通用参考走不了 Ref2VA，需补到 ≥3 张或改两步走' },
            },
            output: { schema: resultSchema, render: renderResult },
            async execute(args, exec) {
                const a = args;
                ...
                const inferred = audioCount > 0
                    ? ({ mode: 'Ref2VA', pictures: filenames.length, audios: audioCount })
                    : filenames.length >= 3
                        ? ({ mode: 'Ref2VA', pictures: filenames.length })
                        : filenames.length === 2
                            ? ({ mode: 'FL2VA', pictures: 2 })
                            : ({ mode: 'I2VA', pictures: filenames.length });
        ...
        defineTool({
            ...
            description: '把分镜表提交给用户审批。**调用本工具后必须立即结束回合** —— 逐步确认模式下本工具会直接终止本回合（带 concludesTurn），此后任何生成动作都会被门禁拒绝；不要读文件、不要加载 skill、不要生成图片或建资产卡，那些都是获批后的下一步，不是可以并行做的准备工作。等用户在画布上方点「批准并开始制作」或「驳回，继续修改」，获批后用户会发「继续」。放手跑模式（auto）直接放行，不停回合。',
            parameters: {
        ...
        defineTool({
            ...
            description: '把剧本提交给用户审批。**调用本工具后必须立即结束回合** —— 逐步确认模式下本工具会直接终止本回合（带 concludesTurn），此后任何生成动作都会被门禁拒绝；不要读文件、不要加载 skill、不要开始规划分镜，那些都是获批后的下一步。等用户在画布上方点「批准剧本」或「驳回，继续修改」，获批后用户会发「继续」。放手跑模式（auto）直接放行进入分镜规划。',
            parameters: {
        ...
        defineTool({
            ...
            parameters: {
                ...
                timesignature: { type: 'string', description: '拍号：2 / 3 / 4（=4/4）/ 6（后端可选值即这四种）；软提示，不接受时自动忽略' },
                sourceUrls: { type: 'array', description: '可选：关联的画布产物 URL 数组（画血缘箭头）' },
                sourceNodeIds: { type: 'array', description: '可选：显式关联的画布节点 id 数组（与 sourceUrls 取并集，过滤不存在的 id）' },
            },
