...
**每一张参考图都应在简报里出现一次**（单独立条目，或在 `<Subject N>` 里 inline 注明出处）。
工具侧的本地预检会检查这件事：编号跳号 = 有一张素材没被引用 —— **只出一句提示、不拦截生成**，
但那意味着那张参考图对本次生成没有起作用。两条出路：补上引用，或把它从 `filenames` 里去掉。
...
工具参数是 `videoRefs`（**有序数组，顺序即 `<Video N>` 的引用序**，不得重排）；
填画布视频节点的 `@ref[显示名]`，或 `upload_video` 返回的**句柄**。
...
| 单段大小 | ≤ 50MB |
| 组合 | **可以作为唯一输入**（与音频相反——只限制音频不能唯一） |

⚠️ **生成产物名不能直接入参**（形如 `MiniMax_H3_ref2va_00020_.mp4`）——实测被后端
0.1s 内笼统 500 拒掉（与图片的 CV-155 同型）。要当参考视频用，它必须先是**上传件**
（上传拿到 `ref-xxxxxxxx.mp4` 形态的句柄）。

⚠️ 跨模态还有一条**文件总数上限**：图 + 视频 + 音频合计 **≤ 12** 个
（图 ≤9 / 视频 ≤3 / 音频 ≤3，单看每路都不超、合起来会超）。
