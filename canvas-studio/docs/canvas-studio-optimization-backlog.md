...
- [ ] **CV-110 确认式澄清（对齐市面产品交互）**：把需求澄清「三连问」（形态→风格大类→具体风格）改为一张「方案确认卡」——agent 先按用户输入推断形态+风格+画幅+时长，用 `ask_user_choice`（或改造后的新卡片）出一张可逐项修改的摘要卡：点「就这样」直接进分镜规划，点单项弹出该要素的候选项修改。动机：①用户零输入时就被要求做抽象选择题（大类标签）体验差；②即梦/Kling 等产品均为「风格即入口+先给推断可改」模式，无开放题；③总纲已有「放手跑」模式（跳过提问列假设），确认卡是逐步模式与放手跑之间的中间档。涉及：`canvas-studio-creation/SKILL.md` 澄清流程重写 + `ask_user_choice` 契约扩展（或新工具 `confirm_plan`）+ `question-capture.tsx` 卡片改版（分组+逐项编辑）。前置依赖：O2 选项高亮改版先落地。**状态**：`[独立排期]`（2026-09-08 讨论立项；同日已落地的低成本缓解见 CV-109：推荐决策表 + 已点名风格豁免）
