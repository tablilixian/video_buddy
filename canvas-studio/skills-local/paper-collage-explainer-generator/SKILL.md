...
- Default video generation model: `MiniMax-H3` (fixed for this project; no model-selection step)
...
5. When generating video clips, request synchronized tactile collage SFX if MiniMax H3 audio is enabled: paper pieces sliding, popping, pressing flat, soft taps, and light paper rustles.
...
- Use the approved still as the visual reference / final-frame anchor
...
Video generation always uses `MiniMax-H3` (fixed model for this project). Do not ask the user to choose a model and do not offer other model options; on failure, retry or loosen the request instead of switching models.
