...
**功能**：**视频理解** —— 把一段视频交给 Qwen3-VL（`qwen3vl_video_analyze.json`），拿回**按时间轴 / 逐镜头**的文字分析：景别、机位运动、节奏、主体动作、光线变化。适合拆解用户上传的参考片、把已生成的镜头片段复述成文字以便写下一镜 prompt、核对成片运镜是否符合预期。探针实测：上传句柄 200 / **16.7s**（1.65MB 样片），产物名 0.1s 前置 500 —— 见 `docs/api-probe/video2vl-20260922/report.md`。

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `video` | string | 是 | 视频**句柄**：画布视频节点的 `@ref[显示名]`（Host 换名）或 `upload_image` 返回名。⚠️ 产物名（`MiniMax_H3_*.mp4`）不能直接传 |
| `mode` | string | 否 | `shot-breakdown`（**缺省**，用官方分镜拆解模板）/ `free`（自由问答，`prompt` 原样发出） |
| `prompt` | string | 否 | `free`：完整问题（**必填**）。`shot-breakdown`：**可选的一句额外关注点**（附在模板之后）。⚠️ **不要把模板本身抄进来** |
| `systemPrompt` | string | 否 | 角色设定（缺省即官方模板配套的「电影分镜分析设计师」） |

**行为要点**：
- 🔑 **好输出的关键：模板不经过模型的手**。缺省模式发出的 `prompt` 是 `src/video-analysis.ts` 里的 **`VIDEO_SHOT_BREAKDOWN_PROMPT` 常量原文**（后端同事按 Qwen3-VL 实测调优：角色设定 + 九项字段〔分镜时间点 / 主体画面 / 运镜方式 / 镜头作用 / 运动节奏 / 人物与关键物件 / 景别 / 音效 / 新画面即切镜〕+ 「直接输出、不要注释」收口），`system_prompt` 是 `VIDEO_ANALYST_SYSTEM_PROMPT`。**让模型照抄长模板必漂移**（漏字段、改措辞、加备注），所以模板只存在代码里一处，由测试守住「发出去的就是它」。
- **与 `image2vl` 同一套文件名纪律**：产物名必 500（0.1s 前置拒绝），`@ref` 会主动重传换名，漏判形态再由失败自愈兜住（反查画布节点 → 本地资产重传 → 换名重试一次）。两条路径与 `image2vl` **共享同一份实现**（`healReferenceFilename` + `isBadReferenceError`）。
- **超时取视频档 600s**（`DRAMA_TIMEOUT_MS.video`），不是文本档 180s —— 耗时随片长增长，180s 对长片不够；`callDramaRaw` 因此新增了超时参数。
- **占用后端单任务槽位**（描述里带 `DRAMA_SERIAL_HINT`）：理解期间别的生成只能排队。
