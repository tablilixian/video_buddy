...
9. **Bug 修复(2026-08-31,已实现待桌面验收)**:
   ...
   - **项目目录名 = UUID(根因)**:`ProjectRegistry.create` 的 `dir = projects/<randomUUID>`。修复:`src/projects.ts` 新增 `sanitizeProjectDirName()`(纯函数,保留中文、非法字符替换 `-`、去首尾点/连字符、Windows 保留设备名加前缀、UTF-8 字节截断 ≤200、空回退 `project`)+ `uniqueDirName()`(sanitize 碰撞追加 `-2/-3…`)；目录 = `projects/<用户名>`;`dirOf()` 优先取 registry 记录 `dir` 字段(兼容历史 UUID 目录),`projectDir/assetsDir/canvasFile` 全部改走 `dirOf`(调用点零改动)。id 仍是内部稳定引用。旧项目不受影响。
   - 验证链全绿(141/141,新增 projects-dir 9 用例)。桌面验收点:① 设置→外观→切主题,画布/对话区/品牌色**立即**变(不用重启);② 新建项目→磁盘 `$DSH_HOME/canvas-studio/projects/<项目名>`(中文名可读),同名/sanitize 碰撞自动加后缀。
10. **Boot 失败修复 — 槽注册竞态(2026-08-31,18:08)**:用户桌面报 "Renderer boot failed for 2 plugin(s)"(canvas-studio + ui-conversation)。**根因**:`ctx.slots.register({ name: 'conversation.hero.brand.mark' }, …)` 在 canvas-studio apply 里**同步**调用,但该槽由 ui-conversation 包声明 —— cordis fiber 加载顺序不保证 ui-conversation 必先于 canvas-studio apply。若 conversation 槽的 children 尚未声明,`SlotCore.register` 抛 "registering into an undeclared slot throws" → canvas-studio fiber crash → 渲染进程 abort。**修复**:`ctx.slots.inject(key, callback)` 等目标槽声明后再同步执行 callback —— 已声明则立即触发,未声明则等声明时再触发,无竞态。同时将类型 cast 改为内联 facade (`as unknown as { inject: (key: string, callback: () => () => void) => () => void; register: (options: { name: string }, component: unknown) => () => void }`),绕开 SlotRegistry 字段类型 narrow 到 'root'(augment 已合并 SlotMap,但 SlotRegistry 字段未刷新)。**类型层面**仍 augment SlotMap 加 'conversation.hero.brand.mark' + 本地 HeroBrandMarkOwnerProps。验证链全绿(141/141),用户重启桌面恢复。
11. **画布工具栏按组隐藏入口(2026-08-31,18:24)**:用户要求只隐藏框选的那几个按钮(非整条工具栏)——**功能全部保留,仅入口隐藏**。**做法**:`canvas/CanvasToolbar.tsx` 新增模块级 `TOOLBAR_VISIBILITY` 常量,9 组各自独立开关,组件与回调全部保留(无死代码)。当前配置:
    - **隐藏**(用户 2026-08-31 18:31 指定):`undoRedo`(撤销/重做)、`editing`(删除/编组/解组)、`create`(+便签/+文本/+提示)
    - **保留显示**:`arrange`(整理布局)、`upload`(~~上传图片/上传视频~~ **2026-09-24 CV-241 起为单入口「上传文件」**，四类自动分类)、`layers`(显示图层)、`zoom`(百分比/−/+/适配/1:1)、`minimap`(显示小地图)、`settings`(齿轮)
    - **替代入口**:节点右键菜单(删除/复制/粘贴/重命名/锁定/置顶置底/重试/改提示词等)、快捷键(Ctrl+Z/Ctrl+Shift+Z 撤销重做仍由画布监听器处理)、画布滚轮缩放、左下角齿轮设置
