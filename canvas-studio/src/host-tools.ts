...
export function createStudioTools(registry: ProjectRegistry, port: number, cfg?: StudioRuntimeConfig) {
  ...
  return [
    ...
    defineTool({
      ...
      parameters: {
        prompt: { type: 'string' as const, required: true, description: '音频整体描述 tags（情绪/风格/乐器/节奏）' },
        lyrics: { type: 'string' as const, description: '歌词提示词（Verse/Chorus 结构）；纯器乐 BGM 留空' },
        duration: { type: 'number' as const, description: '音频时长（秒），默认 30；BGM 建议与成片时长匹配' },
        bpm: { type: 'number' as const, description: '每分钟节拍数，默认 128' },
