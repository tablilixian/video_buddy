 ...
 */
export function truncateDescription(raw, limit = DESCRIPTION_LIMIT) {
    if (raw.length <= limit)
        return { text: raw, truncated: false, dropped: 0 };
 ...
 */
export function collectSkillStats() {
...
export function registerMinimaxSkills(ctx) {
    ...
    const disposers = stats.map((stat) => ctx.skills.register({
        name: stat.name,
    ...
    console.info(`[canvas-studio] skills registered: ${stats.length} total, ${truncated.length} description truncated` +
