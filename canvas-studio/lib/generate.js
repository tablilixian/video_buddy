...
import { DEFAULT_RESOLUTION, DRAMA_ENDPOINTS, isVideoResolution, newAssetId, sizeForAspectRatio, } from './config.js';
 ...
 */
function resolutionOf(params) {
    if (isVideoResolution(params.resolution))
...
async function dramaPost(endpoint, init, timeoutMs, signal) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        const requestInit = { ...init, signal: composed };
        ...
        try {
            ...
            if (timeout.aborted) {
                throw new Error(`Drama Backend ${Math.round(timeoutMs / 1000)}s 内未返回结果（已放弃重试）：` +
...
export async function uploadBytesToDrama(bytes, ext, signal) {
    ...
    if (!upload.ok)
        throw new Error(`文件上传失败: ${upload.status}`);
    ...
    const filename = (data.name
        ...
        ?? data.data?.url);
    if (!filename)
        throw new Error(`文件上传成功但未返回 filename（响应: ${JSON.stringify(data)}）`);
    return filename;
}
/**
 ...
 */
export async function overwriteNodeAsset(registry, projectId, retryOf, patch) {
    const nodes = (await registry.readCanvas(projectId)).nodes;
    const target = nodes.find((node) => node.id === retryOf);
    if (target === undefined)
        throw new Error(`重试目标节点不存在: ${retryOf}`);
 ...
 */
async function replayDetachedAsset(registry, tool, projectId, params, signal) {
...
export async function generateAsset(registry, tool, projectId, params, signal) {
    ...
    if (tool === 'image_generate') {
        ...
        if (providedRefs.length > refs.length) {
            ...
            warnings.push(`参考图最多 4 张生效（后端 image2image 仅 image1~image4），本次已忽略后 ${dropped.length} 张：${dropped.join('、')}`);
        }
        ...
        const _r = await callWithFallback(DRAMA_ENDPOINTS.image2fix, { prompt: params.prompt, image: params.filename }, 'image');
        mediaUrl = _r.url;
        if (_r.filename !== undefined)
            dramaFilename = _r.filename;
    }
...
export async function generateCharacterSheet(registry, projectId, params, signal) {
    ...
    if (!sheetDownload.ok)
        throw new Error(`三视图拼图下载失败: ${sheetDownload.status}`);
...
export function normalizeLookCardName(name) {
 ...
 */
export async function registerLookCard(registry, projectId, params) {
    ...
    if (missing.length > 0) {
        warnings.push(`lockedPrompt 里没解析到这些 Look 字段：${missing.join('、')}`
    ...
    if (params.anchorNodeId !== undefined) {
        if (canvas.nodes.some((node) => node.id === params.anchorNodeId))
            anchorNodeIds.push(params.anchorNodeId);
        else
            warnings.push(`锚点节点 ${params.anchorNodeId} 不在当前画布上，本卡暂无可视锚点（tokens 文字注入不受影响）。`);
    }
    else if (params.anchorRef !== undefined) {
        warnings.push(`没能把锚点「${params.anchorRef}」对应到画布节点（本卡暂无可视锚点，tokens 文字注入不受影响）。`
            + '请确认该素材已在画布上；推荐直接传 @ref[节点标题]。');
    }
    // 同名覆盖：先把不再属于本卡的旧锚点摘干净，再写卡（与 character_sheet 同一语义）。
    if (slot.replacing)
        await registry.releaseAssetNodes(projectId, slot.id, anchorNodeIds);
    await registry.upsertAsset(projectId, {
        id: slot.id,
        name,
        role: 'style',
    ...
    const anchors = anchorNodeIds.flatMap((id) => {
