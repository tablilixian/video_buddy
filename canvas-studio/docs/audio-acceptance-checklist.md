...
| **与视频协作层 C 通道**（audio reference） | **代码全通 + 后端已开放**（2026-09-10 后端 0.2.8 更新） | ✅ 现在就能验 |
| **与视频协作层 B 通道**（原生音轨 `generate_audio`） | 代码已透传；该字段**仍未见于后端文档** | ⚠️ 只能验「被拒时提示是否明确」 |
...
**C 通道（正向验收 —— 后端 2026-09-10 已开放）**
1. 备一段 **2–15s 的 mp3/wav（合计 ≤15s）** —— 画布音频节点或上传皆可；
   再加**至少一张参考图**（官方硬规则：音频不能作唯一输入）。
2. 对 agent 说：**「用这张参考图和这段音乐生成一段 5 秒视频，节奏跟着音乐走」**。
3. 看请求体：音频应落在 **`audio1`**（顺序即提示词里 `<Audio N>` 的引用序），
   且能力解析走 **multi-reference（Ref2VA）**、不是首尾帧。
4. 看成片：动作 / 节奏应跟得上音乐；**画面里不应出现参考图本身**（它是参考，不是首帧）。
5. 再跑一次不合规输入，应**在生成前就报错**：4 段音频 / 合计 16s / 单段 20s / 传 `.m4a` / 只给音频没给图。

**B 通道（只能验「被拒时说得清不清楚」）**
1. 显式请求原生音轨（`generateAudio: true`）→ 看请求体里是否带上 `generate_audio`。
2. 若被拒，错误信息应把**音频参数显式点出**（CV-129 特意做的，避免笼统 500 被误读成参考图失效）。
...
| 音频参考报笼统 500 | ① 后端已开放该字段（2026-09-10），**不再预期必然失败**——先查错误里有没有点出音频参数，再看是不是后端 500 通病；② 也可能踩到 undici 300s 隐形上限（见 av-timeline-plan.md §4.4 / CV-133） |
