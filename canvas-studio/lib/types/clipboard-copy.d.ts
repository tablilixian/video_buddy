...
export declare const CLIPBOARD_LABELS: Readonly<Record<ClipboardPayload, string>>;
/** 成功提示里的名词。 */
export declare const CLIPBOARD_NOUNS: Readonly<Record<ClipboardPayload, string>>;
/**
 * 文字节点该复制出去的正文。
 *
 ...
 */
export declare function clipboardTextOf(node: StudioCanvasNode): string;
/**
 * 一个节点的复制计划（唯一判定）。
 *
 ...
 */
export declare function clipboardPlanOf(node: StudioCanvasNode): ClipboardPlan | null;
/** 剪贴板只收 PNG：非 PNG（webp / jpeg / 未知类型）一律要重编码。 */
export declare function pngTranscodeNeeded(blobType: string): boolean;
/** 失败发生在哪一步。 */
...
export declare function errorTextOf(cause: unknown): string;
/** 按阶段 + 错误名归类。 */
export declare function classifyClipboardFailure(stage: ClipboardStage, cause: unknown): ClipboardFailure;
/**
 * 把节点内容写进系统剪贴板（唯一入口）。
 *
 ...
 */
export declare function copyNodeToClipboard(node: StudioCanvasNode, env: ClipboardEnv): Promise<ClipboardResult>;
/**
 ...
 * 「已复制」，控制台里留一条没人看的报错）。
 */
export declare function copyTextToClipboard(text: string, env: ClipboardEnv): Promise<ClipboardResult>;
/**
 * 结果 → 给用户看的一句话（toast）。
 *
 ...
 */
export declare function clipboardResultMessage(result: ClipboardResult): string;
