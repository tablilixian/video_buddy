...
10. **成片合成**：读 `references/shot-format.md`「成片合成与自检」——compose_video 拼接已有片段（缺省只收有效片段，≥2 段；失效版本自动排除），可传 clipIds / bgmNodeId / scriptId；统一调色与 BGM 淡入淡出默认开启。**严禁再用 video_generate / video_composite 从图片重新生成视频——成片只由已有片段拼接而成。**
