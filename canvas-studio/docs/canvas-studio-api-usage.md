...
| 宿主队列（CV-220） | **所有同步 Drama 调用现在都排同一条宿主队列**（拦截点是 `generate.ts#callDrama` —— 同步生成的唯一网络入口；fal / upload / 文本工具 / health 不走它）。**并发只排队、墙钟不变**：后端本来就一次只做一个，队列的价值不是提速，而是让「排队」**可见**并且**不计入客户端占位截止**（旧实现里排在后面的请求会被误判「生成超时」）。队列态可查：`GET /canvas-studio/generate-queue` → `{active, waiting[]}`（只读、回环同源） |
...
- 2026-09-21 补记（**宿主侧生成队列**；CV-220）：把「后端同步单任务」从**文档纪律**升为**宿主结构**。**根因是误判不是慢**：客户端占位截止 `PENDING_TIMEOUT_MS = 660_000` **从占位落卡起算**，而视频档 `DRAMA_TIMEOUT_MS.video = 600_000` ⇒ **只剩 60s 余量**，只要与任何东西重叠（本端重试 / 重出 / 模型同回合多调用 / 后端被人占满 —— CV-218 B 轮实测 `queue_task_count = 5`），排队的请求「还没轮到」就被判「生成超时」。**落地**：① 拦截点＝`generate.ts#callDrama`（**同步生成的唯一入口** ⇒ 「只拦同步 Drama」是结构性保证，不需要 provider 字符串分支）；② 新建 `src/generate-queue.ts`（Host 进程级队列实例，`finally` 释放）+ `src/queue-view.ts`（纯视图、不持实例 ⇒ 可进浏览器 bundle）；③ 新增**只读**路由 `GET /canvas-studio/generate-queue`（**canvas-studio 宿主路由，不是 Drama 端点**）→ 客户端 2s 轮询 → 画布窄条「生成队列：<标签> · 等待 N 个」；④ **排队计入延期**：拿到非 null 队列态就重挂全部 pending 计时器。**刻意不做**：不接 `maxParallel`（后端单任务 ⇒ 并发无收益，恒为 1，设置页继续标注「不会生效」）；不做「第 N 位」（缺 `requestId → 队列条目` 归属通道 ⇒ 假精度）。§1.2 新增一行「宿主队列」条目。验收用例见 `acceptance-test-cases.md` **Q 组**（Q-1~Q-6）。
